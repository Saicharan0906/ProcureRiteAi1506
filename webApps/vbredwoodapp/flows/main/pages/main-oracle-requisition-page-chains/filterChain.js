...
class FilterChain extends ActionChain{async run(context){
  const {$page}=context;
  if(!$page.variables.reqLinesAllArray||!$page.variables.reqLinesAllArray.length){$page.variables.reqLinesAllArray=[...$page.variables.reqLinesArray];}
  const all=$page.variables.reqLinesAllArray;
  ...
  let rows=[...all];
      ...
      if(selected.reqNumber) rows=rows.filter(r=>r.reqNumber===selected.reqNumber);
  if(keyword){const words=String(keyword).toLowerCase().split(/\s+/).filter(Boolean);const KW=["reqNumber", "itemNumber", "itemDescription"];rows=rows.filter(item=>words.some(w=>KW.some(f=>item[f]&&String(item[f]).toLowerCase().includes(w))));}
  $page.variables.reqLinesArray=rows;
}}return FilterChain;});
