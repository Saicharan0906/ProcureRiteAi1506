...
class FilterChain extends ActionChain{async run(context){
  const {$page}=context;
  if(!$page.variables.exceptionsAllArray||!$page.variables.exceptionsAllArray.length){$page.variables.exceptionsAllArray=[...$page.variables.exceptionsArray];}
  const all=$page.variables.exceptionsAllArray;
  ...
  if(keyword){const words=String(keyword).toLowerCase().split(/\s+/).filter(Boolean);const KW=["itemNumber", "itemDescription", "exceptionType"];rows=rows.filter(item=>words.some(w=>KW.some(f=>item[f]&&String(item[f]).toLowerCase().includes(w))));}
  $page.variables.exceptionsArray=rows;
}}return FilterChain;});
