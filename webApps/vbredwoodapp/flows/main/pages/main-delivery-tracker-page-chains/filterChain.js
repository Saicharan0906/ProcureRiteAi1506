...
class FilterChain extends ActionChain{async run(context){
  const {$page}=context;
  if(!$page.variables.deliveryAllArray||!$page.variables.deliveryAllArray.length){$page.variables.deliveryAllArray=[...$page.variables.deliveryArray];}
  const all=$page.variables.deliveryAllArray;
  ...
  $page.variables.deliveryArray=rows;
}}return FilterChain;});
