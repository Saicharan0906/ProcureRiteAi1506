...
class FilterChain extends ActionChain{async run(context){
  const {$page}=context;
  if(!$page.variables.poLinesAllArray||!$page.variables.poLinesAllArray.length){$page.variables.poLinesAllArray=[...$page.variables.poLinesArray];}
  const all=$page.variables.poLinesAllArray;
  ...
  $page.variables.poLinesArray=rows;
}}return FilterChain;});
